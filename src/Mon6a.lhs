...
>             return (filter (`notElem` vs) (freeVariables x))
...
TODO: monadic expressions?  I'm skeptical of capturing monadic-ness at this point, however... why
not treat expressions uniformly, and have Invoke baked into the treatment of Bind and the top-level
evaluator?
...
> exprTail (X.EBind _ _ _ _ v e k) kt = error "Ain't figured out no monads."
...
>           classifyBranch (X.MGuarded (X.GFrom (X.PCon ctor _ _ vs) _) m) = Right (ctor, vs, m)
...
primitives =
    [-- Bitdata and bit vectors
     ("constructBitdata", constructBitdata),
     ("bitdataSelect", bitdataSelect),
     ("primBitdataEquals",
      \ [_, TyNat size] ->
      let size' = fromIntegral size
          mask = complement (-1 `shiftL` size')
      in vfun $ \(VCon _ [VBits bits _]) -> vfun $ \(VCon _ [VBits bits' _]) ->
           if (bits .&. mask) == (bits' .&. mask)
           then return (VCon "True" [VBits 1 1])
           else return (VCon "False" [VBits 0 1])),
     ("primCompareBit",
      \ [TyNat size] ->
      let size' = fromIntegral size
          mask = complement (-1 `shiftL` size')
      in vfun $ \(VBits bits _) -> vfun $ \(VBits bits' _) ->
           return (case compare (bits .&. mask) (bits' .&. mask) of
                     LT -> VCon "LT" []
                     EQ -> VCon "EQ" []
                     GT -> VCon "GT" [])),
     (":#",
      \ [_, _, TyNat finalSize] ->
      vfun $ \(VBits bits _) -> vfun $ \(VBits bits' rightSize) ->
        return (VBits ((bits `shiftL` rightSize) .|. bits') (fromIntegral finalSize))),
     -- In-built monad
     ("primReturnM",
      \ [t] ->
     ...
     -- Arithmetic
     ("primFromLiteralBit",
      \ [TyNat val, TyNat size] ->
        vfun (\_ -> return (VBits val (fromIntegral size)))),
     ("primBitPlus", bitArith (+)),
     ("primBitMinus", bitArith (-)),
     ("primBitTimes", bitArith (*)),
     -- Ix types
     ("primIxFromLiteral",
      \ [TyNat val, TyNat size] ->
        vfun (\_ -> return (VIx val))),
     ("primModIxMod",
      \ [TyNat bound, TyNat size] ->
        let size' = fromIntegral size
            mask = complement (-1 `shiftL` size')
        in vfun $ \(VBits bits _) -> return (VIx ((bits .&. mask) `mod` bound))),
     ("primIxToUnsigned",
      \ [TyNat bound] ->
        let mask = complement (-1 `shiftL` 32)
        in vfun $ \(VIx n) -> return (VCon "Unsigned" [VBits (n .&. mask) 32])),
     ("primIncIx",
      \ [TyNat bound] ->
        vfun $ \(VIx n) ->
          let n' = n + 1
          in if n' >= bound then return (VCon "Nothing" []) else return (VCon "Just" [VIx n'])),
     -- Booleans
     ("primBoolAnd",
      \ [] -> vfun $ \left -> vfun $ \right ->
                case left of
                  VCon (Ident "True" _ _) _ -> return right
                  VCon (Ident "False" _ _) _ -> return left),
     ("primBoolOr",
      \ [] -> vfun $ \left -> vfun $ \right ->
                case left of
                  VCon (Ident "True" _ _) _ -> return left
                  VCon (Ident "False" _ _) _ -> return right),
     ("primBoolXor",
      \ [] -> vfun $ \left -> vfun $ \right ->
                case left of
                  VCon (Ident "True" _ _) _ -> return (boolNot right)
                  VCon (Ident "False" _ _) _ -> return right),
     ("primBoolNot",
      \ [] -> vfun $ \b -> return (boolNot b)),
     -- Areas
     ("initStored",
...
constructBitdata  :: PrimImpl
constructBitdata [t] = iter [] =<< fields ctorName
    where (args, result) = flattenFunctionType t
          (_, [TyCon typeName, TyLabel ctorName]) = flattenType result
          extendBits (bits', size') (VBits bits size) =
              ((bits `shiftL` fromIntegral size') .|. bits', size + size')
          extendBits (bits', size') (VCon _ [VBits bits size]) =
              ((bits `shiftL` fromIntegral size') .|. bits', size + size')
          iter (VBits bits size : vs) [] = return (uncurry VBits (foldl extendBits (bits, size) vs))
          iter (VCon _ [VBits bits size] : vs) [] = return (uncurry VBits (foldl extendBits (bits, size) vs))
          iter vs (LabeledField _ _ _ _ : fs) = vfun (\v -> iter (v : vs) fs)
          iter vs (ConstantField bits pat _ : fs) = iter (VBits bits (BDD.width pat) : vs) fs

bitdataSelect :: PrimImpl
bitdataSelect [TyCon _, TyLabel ctor, TyLabel field, resultType] =
    vfun (\(VBits bits inputSize) -> vfun (\_ -> selectFrom bits inputSize))
    where selectFrom bits inputSize =
              do LabeledField _ _ pat offset <- lookupField `fmap` fields ctor
                 let mask = complement ((-1 :: Integer) `shiftL` BDD.width pat)
                 interpretBitPattern ((bits `shiftR` offset) .&. mask) (BDD.width pat) resultType

          lookupField (f@(LabeledField field' _ _ _) : rest)
              | field == field' = f
              | otherwise = lookupField rest
          lookupField (ConstantField _ _ _ : rest) = lookupField rest
          lookupField _ = evalError ("Missing field " ++ fromId field ++ " in bitdata constructor environment.")

          interpretBitPattern bits _ (TyApp (TyCon (Kinded (Ident "Bit" _ _) _)) (TyNat resultSize)) =
              return (VBits bits resultSize')
              where resultSize' = fromIntegral resultSize
          interpretBitPattern bits size (TyCon (Kinded resultType _)) =
              do (_, cenv) <- ask
                 let (ctor:_) = [cname | (tname, cname, Right fields) <- cenv, tname == resultType, patFrom cname fields `BDD.willMatch` bits]
                 return (VCon ctor [VBits bits size])
              where patFrom cname fields = BDD.pSplits (map patFromField fields)
                    patFromField (LabeledField _ _ pat offset) = pat
                    patFromField (ConstantField value pat offset) = pat
...
boolNot :: Value -> Value
boolNot (VCon (Ident "True" _ _) _) = VCon "False" [VBits 0 1]
boolNot (VCon (Ident "False" _ _) _) = VCon "True" [VBits 1 1]
...
>     loop decls >> exprTail main (return . Done)
